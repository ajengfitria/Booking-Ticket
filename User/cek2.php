		...
		<div class="banner" style="background: #FFF;">
				...
				<div class="col-md-12">
					<div class="sap_tabs" style="background-color: rgb(97, 102, 125); border-radius: 0px;">
						...
						<table class="table" style="border:none; color: #FFF;">
							...
							$data = mysqli_query($cn, "SELECT pemesan.kode_res, pemesan.id_pemesan ,pemesan.status,pemesan.id,rute.asal,rute.tujuan,rute.tgl,rute.sisa,rute.berangkat,rute.sampai,rute.bandara_asal,rute.bandara_tujuan,rute.harga,rute.id_rute,transport.nama_trans,transport.jml FROM rute, transport, pemesan where pemesan.kode_res='$kode_res' AND pemesan.id=rute.id_rute AND rute.id=transport.id_trans");
			...
			<div class="container">
						...
						<form method="POST" enctype="multipart/form-data">
							<div class="panel-body" style="border-radius: 0px;">
								...
								<div class="form-group">
									...
									<input type="file" style="border-radius: 0px;" class="form-control" name="bukti_bayar">
								</div>
								<input type="hidden" name="id_pemesan" value="<?php echo $d['id_pemesan'] ?>">
								<input type="hidden" name="kode_res" value="<?php echo $kode_res ?>">
								<div class="form-group">
									<div class="date_btn" style="float: right;">
				...
				if (isset($_POST['submit'])) {
					...
					$ft = move_uploaded_file($tmp_name, 'images/'.$bukti_bayar);
					$update = $cn->query("UPDATE pemesan SET status='Menunggu Konfirmasi', bukti_bayar=('".$bukti_bayar."') WHERE id_pemesan='$id_pemesan'");
					header('Location:cek3.php?'.$getdata);
				}
				?>
			</div>
			<!-- //container -->
